...
class SystemMonitor:
    ...
    def get_network_speed(self):
        """Calculate real-time network speed in MB/s"""
        try:
            current_time = time.time()
            current_net_io = psutil.net_io_counters()
            
            if self.last_net_io is not None and self.last_time is not None:
                ...
                if time_diff > 0:
                    ...
                    # Convert to MB/s
                    speed_mbps = (total_bytes_diff / time_diff) / (1024 * 1024)
            ...
            # First run or error case - store current values
            self.last_net_io = current_net_io
            self.last_time = current_time
            return 0.0
            ...
            while True:
                ...
                if data:
                    ...
                    if self.send_data(data):
                        # Print status with network speed in appropriate units
                        net_speed = data['network']
                        if net_speed < 1.0:
                            net_display = f"{net_speed * 1024:.0f}KB/s" if net_speed > 0 else "0KB/s"
                        else:
                            net_display = f"{net_speed:.1f}MB/s"
