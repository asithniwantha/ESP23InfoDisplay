...
void LVGLStyles::cleanup() {
    if (!styles_initialized) return;
    
    lv_style_reset(&style_card);
    lv_style_reset(&style_progress);
    lv_style_reset(&style_chart);
    
    styles_initialized = false;
}

lv_color_t LVGLStyles::getColorForValue(float value, int warning_threshold, int danger_threshold) {
